...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 try{
  if($action==='register'){
   ...
   $s->execute([$me['id'],trim($_POST['title']??''),trim($_POST['authors']??''),trim($_POST['abstract']??''),trim($_POST['keywords']??''),$_POST['section']??'Research Article',$_POST['language']??'English',trim($_POST['references']??'')]);
